...
class Save_Prediction_for_eval_iou(object):
    ...
    def run(self):
        ...
        save_pickle(f'{self.cfg.dir["out"]}/{self.test_mode}/results/iou/shape_list', self.shape_list)
...
class Save_Prediction_for_eval_acc(object):
    def __init__(self, cfg=None):
        self.cfg = cfg

    def get_2D_lane_points(self, data):
        out_num = len(data)
        pred = list()
        for i in range(out_num):
            x_coord = data[i]
            pts = np.concatenate((x_coord.reshape(-1, 1), self.py_coord), axis=1)
            if self.use_height == True:
                pts = pts[self.height_idx[0]:self.height_idx[1]]
            if len(pts) <= 3:
                continue
            pts[:, 0] = pts[:, 0] / (self.cfg.width - 1) * (self.org_w)
            pts[:, 1] = (pts[:, 1] + self.cfg.crop_size) / (self.cfg.height + self.cfg.crop_size - 1) * (self.org_h)
            pred.append(pts)
        self.pred = pred

    def write_pred_data(self):
        save_pickle(f'{self.output_dir}/{self.file_name}', self.pred)
    ...
    def load_pred_data(self):
        ...
        self.out = data['out']
        self.out_x_coord = self.out[self.key[0]]
        if len(self.out_x_coord) != 0:
            self.out_x_coord = to_np(self.out_x_coord)
        self.height_idx = self.out['height_idx']
    ...
    def settings(self, key, test_mode='test', use_height=True):
        ...
        self.output_dir = f'{self.cfg.dir["out"]}/{test_mode}/results/acc/'
        mkdir(self.output_dir)
