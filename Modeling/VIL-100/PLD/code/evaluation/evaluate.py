...
class Evaluation(object):
    ...
    def run_for_miou(self):
        iou = self.measure_IoU(self.b_map, self.seg_map_gt)
        self.results['miou'] += list(to_np(iou))
    ...
    def measure(self):
        ...
        # miou = np.mean(np.float32(results['miou']))
    ...
    def update(self, batch, out, batch_idx=None, prev_frame_num=None, mode=None):
        self.mode = mode
        if prev_frame_num == 0:
            self.seg_map = out['seg_map_init'][batch_idx:batch_idx + 1]
        else:
            self.seg_map = out['seg_map']
        self.b_map = (self.seg_map > self.cfg.prob_thresd).type(torch.long)[:, 0]
        self.seg_map_gt = batch['seg_label'][self.sf[0]][batch_idx:batch_idx + 1]