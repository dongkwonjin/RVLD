...
def save_model(checkpoint, param, path):
    ...
    torch.save({
        ...
        'batch_iteration': checkpoint['batch_iteration'],
...
def save_model_min(checkpoint, path, min_val, val, logger, logfile, model_name):
    if min_val > val:
        save_model(checkpoint, model_name, path)
        min_val = val
        logger("Epoch %03d => %s : %5f\n" % (checkpoint['epoch'], model_name, min_val), logfile)
        print(model_name)
    return min_val
