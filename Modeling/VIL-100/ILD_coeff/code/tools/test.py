...
class Test_Process(object):
    def __init__(self, cfg, dict_DB):
        ...
        self.save_pred_for_eval_acc = dict_DB['save_pred_for_eval_acc']
        self.eval_acc = dict_DB['eval_acc']
    ...
    def evaluation(self, mode):
        ...
        if self.cfg.do_eval_acc == True:
            self.save_pred_for_eval_acc.settings(key=['x_coords'], test_mode=mode, use_height=True)
            self.save_pred_for_eval_acc.run()
            metric.update(self.eval_acc.measure_accuracy(mode))
            metric['err'] = metric['fp'] + metric['fn']
