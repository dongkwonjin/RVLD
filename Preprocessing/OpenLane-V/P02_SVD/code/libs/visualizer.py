...
class Visualize(object):
    ...
    def draw_lanes_for_datalist(self, px_coord, px_coord_ap, py_coord):
        ...
        for i in range(px_coord.shape[1]):
            ...
            if len(node_pts) == 0:
                continue
            self.draw_polyline(data=to_np(node_pts), name='img_overlap', ref_name='img_overlap', color=(0, 0, 255), s=2)
            self.draw_polyline(data=to_np(node_pts), name='label_overlap', ref_name='label_overlap', color=(0, 0, 255), s=2)
